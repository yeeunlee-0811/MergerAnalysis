function [share] = sharefn_pm(theta,inp)
...
% product vars
nj0 = inp.nj0_pm;
pkmat = inp.pkmat_pm; % nj0_pm x npk_pm (4 x 7)
npk = inp.npk_pm; % npk = 7
netflix = inp.netflix_pm;
wavve = inp.wavve_pm;
tving = inp.tving_pm;
wvtv = inp.wvtv_pm;
nettv = inp.nettv_pm;
...
pkmati0 = reshape(pkmat,[nj0,1,npk]); % 4 x 1 x 7
pkmati = repmat(pkmati0,1,n); % 4 x n x npk
...
mu = 0;
for kk = 1:npk
    pki = pkmati(:,:,kk); % nj0 (4) x n x npk
    mu = mu + pki*thpk(kk,1); % nj0 (4) x n
    munu0 = repmat(mu,1,1,R); % nj0 (4) x n x R
end
