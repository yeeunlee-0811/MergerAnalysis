...
for kk = 1:npk
    ...
    munu = munu0 ... 
        ...
        + th1*wavve.*payermat + th2*wavve.*dwvivmat + th3*wavve.*agemat + th4*wavve.*schoolmat + th5*wavve.*djobmat + th6*wavve.*incomemat +th7*wavve.*femalemat + th8*wavve.*hhldmat ... 
        + th9*netflix.*payermat + th10*netflix.*dhnetmat +th11*netflix.*agemat + th12*netflix.*schoolmat + th13*netflix.*djobmat +th14*netflix.*incomemat +th15*netflix.*femalemat + th16*netflix.*hhldmat...
        + th17*tving.*payermat + th18*tving.*dtvivmat + th19*tving.*agemat + th20*tving.*schoolmat + th21*tving.*djobmat+th22*tving.*incomemat +th23*tving.*femalemat + th24*tving.*hhldmat;
